...
import mer.mano.services.CartItemService;
import mer.mano.services.CustomerService;
import mer.mano.services.ProductService;
...
public class CartItemController {
	...
	public String addToCart(@AuthenticationPrincipal Principal principal,  @PathVariable int id,@RequestParam int quantity) {
	 ...
	 public String getCart(@AuthenticationPrincipal String username,Model model) {
		 //String username=principal.getName();
		 User user=customerService.getUser(username);
		 Customer customer=user.getCustomer();
		 Cart cart=customer.getCart();
	...
	@RequestMapping(value="/getcart")	 
	 ...
	 @RequestMapping(value="/cart/clearcart/{cartId}")
		...
		public String createOrder(@PathVariable("cartId") int cartId,@Valid @ModelAttribute ShippingAddress shippingaddress,BindingResult result, Model model){
		...
		public String confirm(@PathVariable("cartId") int cartId){
			Cart cart=cartItemService.getCart(cartId);
			List<CartItem> cartItems=cart.getCartItems();
			for(CartItem cartItem : cartItems){//for(T v:collection)
				cartItemService.removeCartItem(cartItem.getId());//delete from cartitem where id=3
